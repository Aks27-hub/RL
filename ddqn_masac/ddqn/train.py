"""
ddqn/train.py — Full training loop for Double DQN on CityLearn.

Usage:
    python ddqn/train.py --episodes 150 --seed 42 --device auto
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Double DQN on CityLearn")
...
def main():
    ...
    pbar = tqdm(range(1, args.episodes + 1), desc="DDQN Training")
    for episode in pbar:
        obs = env.reset()
        episode_reward = 0.0
        done = False
        ...
        while not done:
            ...
            actions = agent.act(obs, epsilon=agent.epsilon)
            next_obs, rewards, done, info = env.step(actions)
            total_reward = sum(rewards)

            # Store transition
            agent.store(obs, actions, total_reward, next_obs, done)

            # Update only every UPDATE_EVERY steps to reduce compute
            if step_in_episode % UPDATE_EVERY == 0 and agent.replay_buffer.size >= args.batch_size:
                losses = agent.update()
        ...
        # Extract metrics for this episode
